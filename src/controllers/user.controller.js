...
export const postRegistro = async (req, res) => {
  try {
    ...
    console.log(nombre);
    const connection = await pool.getConnection();
    const rows_insert = await connection.query('INSERT INTO cliente (id_cliente, nombre_cliente, telefono_cliente, direccion_cliente, email_cliente, password_cliente) VALUES (?, ?, ?, ?, ?, ?)', [id, nombre, telefono, direccion, email, password]);
    connection.release();
...
export const LoginPost = async (req, res) => {
  try {
    ...
    const connection = await pool.getConnection();
    const [rows] = await connection.query('SELECT * FROM cliente WHERE email_cliente = ? ', [email]);
    connection.release();
...
export const RecuperarPost = async (req, res) => {
  ...
  try {
    ...
    const connection = await pool.getConnection();
    const [rows] = await connection.query(`SELECT email_cliente FROM cliente WHERE email_cliente = ?`, [email]);

    if (rows.length > 0) {
      let tokensEmail = Math.floor(Math.random() * 100000);

      const [rows3] = await connection.query(`UPDATE cliente SET token_cliente = ? WHERE email_cliente = ?`, [tokensEmail, email]);
      connection.release();
      await sendEmails(email, 4, tokensEmail);
    }
...
export const Verificar = async (req, res) => {
  try {
    ...
    const connection = await pool.getConnection();
    const [rows] = await connection.query(`SELECT token_cliente FROM cliente WHERE token_cliente = ?`, [token]);

    if (rows.length > 0) {
      const [rows2] = await connection.query(`UPDATE cliente SET password_cliente = ? WHERE token_cliente = ?`, [password, token]);
      connection.release();
      return res.status(200).json({ message: "Contraseña actualizada correctamente" });
    } else {
      return res.status(401).json({ message: "El código de recuperación es incorrecto" });
...
export const updateUsuarioGet = async (req, res) => {
  try {
    const connection = await pool.getConnection();
    const [rows] = await connection.query('SELECT * FROM cliente WHERE id_cliente = ?', [req.userId]);
    connection.release();
    res.render('cliente/edit', { cliente: rows[0] });
  } catch (error) {
...
export const updateUsuarioPost = async (req, res) => {
  try {
    ...
    const connection = await pool.getConnection();
    const [rows] = await connection.query('UPDATE cliente SET ? WHERE id_cliente = ?', [newCliente, id]);
    connection.release();
...
export const viewProfileGet = async (req, res) => {
  try {
    ...
    const connection = await pool.getConnection();
    const [rows] = await connection.query('SELECT * FROM cliente', [req.userId]);
    connection.release();
